...
namespace Axerrio.BB.DDD.Infrastructure.Query.Validation
{
    public class SpecificationValidationSettingsValidator: AbstractValidator<SpecificationValidationSettings>
    {
        ...
        {
            RuleFor(s => s).Must(s => (s.AllowedSpecificationOptions.HasFlag(SpecificationOptions.Filter) || !s.RequiredSpecificationOptions.HasFlag(SpecificationOptions.Filter))).WithMessage("Filtering cannot be both required and not allowed");
